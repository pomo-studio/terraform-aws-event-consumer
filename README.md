...
Subscribe a service to events on a shared EventBridge bus, and process them through an SQS queue that absorbs bursts and owns the retries.

## When to use it

Reach for this module when one service reacts to one class of events. Each consumer gets its own rule, queue, retry policy, and dead-letter queue, so consumers fail independently of each other and of the producer.

Use a different tool when you need to publish events (that is the producer's job), or when a single synchronous call answers the question. This module pairs with [`pomo-studio/event-bus/aws`](https://registry.terraform.io/modules/pomo-studio/event-bus/aws) for the shared bus.
...
- **SQS buffer.** EventBridge hands matched events to SQS, which absorbs bursts and retries independently of Lambda concurrency.
- **Shared IAM role.** One Lambda execution role serves both regions, because IAM is global.
- **ReportBatchItemFailures.** Event source mappings report partial batch failures by default, so one bad record does not replay an entire batch.
- **Log resource policies.** EventBridge-to-CloudWatch logging uses resource-based policies rather than an IAM role on the target.
- **Cross-variable validation.** Terraform 1.9.0 or newer is required to check `lambda_timeout < sqs_visibility_timeout_seconds` at plan time.
...
| <a name="input_bus_name_primary"></a> [bus\_name\_primary](#input\_bus\_name\_primary) | EventBridge bus name in the primary region, typically from the event-bus module output | `string` | n/a | yes |
...
| <a name="input_event_pattern"></a> [event\_pattern](#input\_event\_pattern) | EventBridge event pattern: which events this consumer receives. See the AWS docs for pattern syntax. | `any` | n/a | yes |
